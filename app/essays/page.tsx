    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="yc-container py-20">
        <section className="max-w-4xl">
          <p className="section-kicker mb-6">essays</p>
          ...
          <p className="yc-text-block text-lg leading-8 text-white/70">
            Operating notes for founders, sales leaders, and ambitious early-career candidates. Start with the latest essay or browse the full archive below.
          </p>
        </section>

        {featuredEssay ? (
          <section className="my-20 border-y border-white/10 py-12">
            <Link href={`/essays/${featuredEssay.slug}`} className="group grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
              <div>
                <p className="section-kicker mb-4">latest</p>
                <p className="text-sm text-white/50">
                  {formatEssayDate(featuredEssay.date)} · {featuredEssay.category} · {featuredEssay.readingTime}
                </p>
              </div>
              <div>
                ...
                <p className="max-w-2xl text-lg leading-8 text-white/70">{featuredEssay.description}</p>
              </div>
            </Link>
          </section>
        ) : null}
        ...
        <section className="grid gap-6 md:grid-cols-2">
          {remainingEssays.map((essay) => (
            <Link key={essay.slug} href={`/essays/${essay.slug}`} className="group border border-white/10 bg-white/[0.03] p-8 transition-colors hover:border-primary/60">
              <p className="mb-4 text-xs font-semibold uppercase tracking-[0.24em] text-primary">{essay.category}</p>
              ...
              <p className="mb-8 text-white/65">{essay.description}</p>
              <p className="text-sm text-white/45">
                {formatEssayDate(essay.date)} · {essay.readingTime} · by {essay.author}
              </p>
            </Link>
          ))}
        </section>
        ...
        <section className="mt-20 border border-primary/30 bg-primary/10 p-8">
          ...
          <p className="mb-6 max-w-2xl text-white/70">
            The essays archive is the home for tactical notes. For a more structured reading path, visit the book page.
          </p>
